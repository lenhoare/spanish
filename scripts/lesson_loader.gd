...
static func normalise(d: Dictionary) -> Dictionary:
	...
	for q in d.get("sweets", []):
		if q is Dictionary:
			...
			for k in ["at", "rounds", "timetable", "rooms", "pads", "sign", "character", "greeting", "stalls", "budget", "model", "orders", "dishes", "chef", "chef_says", "fishing_at", "interview", "jobs", "need", "berth_at", "ship_at", "boat", "sections", "places", "stops", "depot_at", "road", "gates", "time"]:
				if q.has(k):
					s[k] = q[k]
			out.sweets.append(s)
			i += 1
